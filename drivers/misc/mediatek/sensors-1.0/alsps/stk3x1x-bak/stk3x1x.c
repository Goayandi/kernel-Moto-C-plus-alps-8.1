...
/*----------------------------------------------------------------------------*/
	...
	#define STK_LT_N_CT	132
	#define STK_HT_N_CT	140
...
extern int stk3x1x_init_flag;	// 0<==>OK -1 <==> fail
...
static ssize_t stk3x1x_show_allreg(struct device_driver *ddri, char *buf)
{
	...
	ret = stk3x1x_master_recv(stk3x1x_obj->client, 0x3E, &rbuf[25], 2);
	if(ret < 0)
	{
		APS_DBG("error: %d\n", ret);
		return -EFAULT;
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	atomic_set(&obj->alsctrl_val, 0x3a);
